export default class TextAreaItemisedRows {
  ...
  init_() {
    ...
    this.$formFieldInput.addEventListener("keyup", () => this.updateList_());
    this.$formFieldNewInput.addEventListener("keypress", (event) => {
      if (event.key === "Enter") {
        this.appendValue_(event.target.value);
      }
    });
    ...
    this.$formFieldNewInputAddButton.addEventListener("click", () => {
      this.appendValue_(this.$formFieldNewInput.value);
    });
  ...
  updateAriaText_(content) {
    this.$formFieldLiveAria.innerText = content;
  }

  /* eslint-disable-next-line class-methods-use-this */
  normaliseValue_(value) {
    return value.replace(/\s{2,}/u, " ").trim();
  }

  isValueInList_(value) {
    return this.values
      .map((eachValue) => this.normaliseValue_(eachValue).toLowerCase())
      .includes(value.toLowerCase().trim());
  }

  appendValue_(value) {
    const newValue = this.normaliseValue_(value);
    if (newValue) {
      if (this.isValueInList_(newValue)) {
        this.updateAriaText_(`'${newValue}' already added`);
      } else {
        this.values.push(newValue);
        this.$formFieldInput.value = this.values.join("\n");
        this.updateList_();
        this.updateAriaText_(`'${newValue}' added`);
      }
    } else {
      this.updateAriaText_("Enter an item to add");
    }
  ...
  removeValue_(value) {
    const removeValue = this.normaliseValue_(value);
    if (removeValue && this.isValueInList_(removeValue)) {
      this.values = this.values.filter(
        (existingValue) => existingValue !== removeValue,
      );
      this.$formFieldInput.value = this.values.join("\n");
      this.updateList_();
      this.updateAriaText_(`'${removeValue}' removed`);
    }
